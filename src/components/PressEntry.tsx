...
export interface PressEntryDoc {
  title: string;
  subtitle?: string;
  slug?: string;
  venue?: string;
...
/**
 * One press entry: title, venue/publication, date/location, copy, "Read more"
 * and photos. Shared by the Press list and each entry's own page, so an entry
 * with no page-builder content still has the same substance on its own page.
 */
export function PressEntry({
  ex,
  id,
  className = '',
  titleAs,
  ...
  id?: string;
  className?: string;
  /** Element for the title; the detail page makes it the page's single <h1>. */
  titleAs?: ElementType;
  /** Where the title links to, if anywhere. */
  titleHref?: string;
  /** Cap on photos shown; omit to show them all. */
  imageLimit?: number;
}) {
  ...
  return (
    <article id={id} className={className || undefined}>
