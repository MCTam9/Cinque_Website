...
function ImageRow() {
  return (
    <div className="grid grid-cols-2 gap-[10px] md:grid-cols-3">
      {IMAGES.map((src, i) => (
        <div key={src} className={`group relative aspect-[2/3] w-full overflow-hidden bg-cloud/30 ${i === 2 ? 'max-md:hidden' : ''}`}>
...
function Meta({ publication, date, location }: { publication: string; date: string; location: string }) {
  return (
    <div className="flex shrink-0 flex-col gap-[10px] sm:items-end sm:text-right">
      <P1 className="text-oslo">{publication}</P1>
      <dl className="flex gap-4 type-p1">
  ...
  return (
    <Container className="py-[40px] md:py-[60px]">
      ...
      <article className="mb-[60px]">
        <div className="mb-[20px] flex flex-col gap-[20px] border-b border-graphite pb-[20px] sm:flex-row sm:items-start sm:justify-between">
          <H2 className="max-w-xl">The_Invisible_Made_Visible</H2>
          <Meta publication="London Craft Week" date="2026-09" location="Blackdot Gallery, London" />
        </div>
        <div className="mb-[30px] flex max-w-2xl flex-col gap-[20px]">
          <P1>
          ...
          <P1>
            ...
            making — from unseen labour and material transformation to the quiet gestures embedded in
            process. Bringing together practices across jewellery, ceramics, sculpture, installation,
            furniture, wearable works, glass, textile and embroidery, the exhibition reveals what
            often remains unnoticed: the time, care and experimentation behind each piece,
            highlighting the stories and processes that usually remain invisible.
          </P1>
        </div>
        <ImageRow />
      </article>
      ...
      <article>
        ...
        <div className="mb-[30px] flex max-w-2xl flex-col gap-[10px]">
          <P1>
            <a
